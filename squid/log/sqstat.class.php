...
class squidstat{
	...
	function squidstat(){
		if (!function_exists("preg_match")){
			$this->errno=5;
			$this->errstr='Você precisa instalar o PHP';
			$this->showError();
			exit(5);
		}
	...
	function formatXHTML($body,$refresh,$use_js=false){
		...
		$text.='<title>Squid em Tempo Real</title>'
		.($use_js?'<script src="zhabascript.js" type="text/javascript"></script>':'').'</head>'
		.($use_js?'<body onload="jsInit();"><div id="dhtmltooltip"></div>':'<body>')
		.$body.'</body></html>';
		return $text;
	}
	...
	*/
	...
	function makeHtmlReport($data,$resolveip=false,$hosts_array=array(),$use_js=true){
		...
		'<br/>'.
		'Atualização automática: <input name="refresh" class="input" type="text" size="4" value="'.$refresh.'"/> sec. <input type="submit" class="botao" value="Iniciar"/> <input name="stop" type="submit" class="botao" value="Parar"/> Iniciado em: <tt>'.date("h:i:s d/m/Y").'</tt><br/>'.
		'</div>'.
		'<table class="result" align="center" width="100%" border="0">'.
		'<tr>'.
		'<th>'.$group_by_name.'</th><th>URL</th>'.
		($this->use_sessions?'<th>Velocidade Atual</th><th>Velocidade Média</th>':'').
		'<th>Tamanho</th><th>Tempo</th>'.
		'</tr>';
		$ausers=$acon=0;
		unset($session_data);
		...
		$stat_row='';
		if($this->use_sessions){
			$stat_row.=sprintf("<tr class=\"total\"><td><b>Total:</b></td><td align=\"right\" colspan=\"5\"><b>%d</b> usuário e <b>%d</b> conexões</td></tr>",
		...
		if($ausers==0){
			$text.='<tr><td colspan=6><b>Não há conexõs ativas</b></td></tr>';
		}
		else {
